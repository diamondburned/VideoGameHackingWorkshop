...
export function AllowedMapBlocks(map: map.Data): Block[] {
    ...
    const check = (_: unknown, block: Block) => {
        const attrs = map.metadata.blockMods.get(block);
        if (attrs && attrs.includes("air")) {
            airBlocks.push(block);
        }
    };

    map.metadata.blocks.forEach(check);
    map.metadata.entities.forEach(check);
    ...
    handleCommand(server: ws.Server, cmd: Command) {
        switch (cmd.type) {
            ...
            case "MOVE": {
                ...
                const block = this.level.map.at(pos.x, pos.y);
